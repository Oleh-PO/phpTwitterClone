	...
	if (document.querySelector(".bottom")) {
		...
		loadPost = function(entries) {
			if (entries[0].isIntersecting) {
				...
				xhttp.open("GET", "/php/user/feedInit.php/?offset="+ty+"&order="+order+"&"+userId);
				xhttp.send(); //ajax recuest to feedInit.php, respond whit post. chang post via postTemplate.php
			}
		}
